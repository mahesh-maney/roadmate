...
export class SidebarComponent implements OnInit {
    ...
    constructor(public menu: MenuService, public settings: SettingsService, 
        ...
        if(!isUndefined(principal) && principal !==null){
            this.setCurrentRole(principal);
        }

        eventManager.subscribe('loggedin',
          (data) => { 
            this.roles = this.setCurrentRole(data.content);
            console.log(menu);  
          }
        );
    }
    ...
    applicableRoles(itemRole){
        if(!isUndefined(itemRole) && itemRole !== null){
            if(!isUndefined(this.roles) && this.roles !== null){
                return this.roles.some(r=>r===itemRole);    
            }else{
                return false;
            }
        }else{
    ...
    setCurrentRole(currUser){
        if(!isUndefined(currUser.userIdentity) && currUser.userIdentity !== null){
         this.roles = currUser.userIdentity.authorities ;
        }
    }
